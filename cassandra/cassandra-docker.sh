...
## we could probably use the service-ca cert, which would cover all certs generated by this OS
## but we are interested only in accepting requests from this Cassandra service, so, we set the
## trust keystore to contain only this cert
SERVICE_CA=${SERVICE_CA:-${SERVICE_CERT}}
...
echo "Importing service CA into the trust store"
